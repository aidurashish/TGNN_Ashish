"""
    ...
    Models:
        ...
        4.  ATMGNN     (no SEIR)   - spatio-temproal GNN with multi-resolution coarsening and temporal self-attention
        5.  DiffATMGNN (no SEIR)   - ablates SEIR in the probabilistic setting
        6.  ATMGNN     (with SEIR) - incorporates SEIR node features
        7.  DiffATMGNN             - propsoed model
...
# same HPO run used to train the full DiffATMGNN model.
...
def _country_worker(packed_args):
    ...
    def _train_step(model, optimizer, adj, feat, y_batch):
        ...
        if model_name == "ATMGNN_noSEIR":
            ...
            # Non-negativity constraint from DiffATMGNN_training.py
